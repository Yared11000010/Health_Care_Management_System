...
<form accept-charset="utf-8" class=" rounded p-3" action="{{ url('admin/updatemedicine') }}" method="POST">
    @csrf
    @method('PUT')
    <ul class="nav nav-tabs align-items-end pb-4 w-100">
        <li class="nav-item border-none">
           <a class="nav-link active bg-light disabled" href=""><i class=" fas fa-plus"></i>Edit Medicine</a>
         </li>
        <li class="nav-item">
   ...
   <input type="hidden" name="id" value="{{ $medicine->id }}">
       <div class="form-group">
        ...
        <input type="text"  name="medicine_name"  value="{{ $medicine->medicine_name }}" placeholder="Enter medicine_name" class="form-control"   />
        ...
        <div class="form-group">
            ...
            <input type="date" name="expiry_date" value="{{ $medicine->expiry_date }}" placeholder="Enter expiry_date" class="form-control"   />
            @error('expiry_date') <span class="text-red-500 text-danger text-xs">{{ $message }}</span> @enderror
            </div>
        <div class="form-group">
            <label for="price">Price</label>
            <input type="number" name="price" value="{{ $medicine->price }}"  placeholder="Enter price" class="form-control"   />
            @error('price') <span class="text-red-500 text-danger text-xs">{{ $message }}</span> @enderror
        </div>
        <div class="form-group">
            <label for="quantity">Quantity</label>
            <input type="number" name="quantity" value="{{ $medicine->quantity }}" placeholder="Enter quantity" class="form-control"   />
            @error('quantity') <span class="text-red-500 text-danger text-xs">{{ $message }}</span> @enderror
        </div>
        <div class="form-group">
            <label for="code">Code</label>
            <input type="text" name="code" value="{{ $medicine->code }}" placeholder="Enter code" class="form-control"   />
            @error('code') <span class="text-red-500 text-danger text-xs">{{ $message }}</span> @enderror
        </div>
    <div class="form-group">
        <input type="submit" class="btn btn-warning" value="Update Medicine">
    </div>
</form>
</div>
</div>
